...
class MyCNNBackbone(nn.Module):
    """用你的 MACNN 做 netF：输出 [B, D] 特征，并提供 in_features。"""
    def __init__(self, in_channels=2, channels=16):
        super().__init__()
        # num_classes 用占位；我们只用 features，不用它的 fc
        self.core = MACNN(in_channels=in_channels, channels=channels, num_classes=1)
        # 你的 MACNN 有 emb_dim；若没有就按 12*channels
        self.in_features = getattr(self.core, "emb_dim", channels * 12)

    def forward(self, x):
        # 只取特征，不过你自己的 fc
        emb = self.core.forward_features(x)  # [B, D]
        return emb
...
class ActiveBaseModel(nn.Module):
    def __init__(self, args):
        ...
        elif args.net == 'mycnn_backbone':  # 新增分支
            netF = MyCNNBackbone(in_channels=getattr(args, 'in_channels', 2),
                                 channels=getattr(args, 'channels', 16))
        else:
